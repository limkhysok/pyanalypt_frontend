...
export function ProjectPage() {
    ...
    const handleCreateProject = async (data: CreateProjectRequest) => {
        try {
            ...
            globalThis.dispatchEvent(new CustomEvent('projects-changed'));
        } catch (error) {
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    >
                                                        <DropdownMenu>
                                                            ...
                                                            <DropdownMenuContent onClick={(e: React.MouseEvent) => e.stopPropagation()} align="end" className="w-60 p-2 shadow-2xl rounded-2xl border-border/40 backdrop-blur-xl">
                                                                <DropdownMenuItem onClick={(e: React.MouseEvent) => { e.stopPropagation(); toggleFavorite(project); }} className="rounded-xl gap-3 py-3 text-sm font-bold">
                                                                ...
                                                                <DropdownMenuItem onClick={(e: React.MouseEvent) => { e.stopPropagation(); openEditModal(project); }} className="rounded-xl gap-3 py-3 text-sm font-bold">
                                                                    <Edit2 className="h-4 w-4" />
                                                                    Configure Workspace
                                                                </DropdownMenuItem>
                                                                <DropdownMenuItem onClick={(e: React.MouseEvent) => { e.stopPropagation(); toggleArchive(project); }} className="rounded-xl gap-3 py-3 text-sm font-bold">
                                                                    <Archive className="h-4 w-4" />
                                                                    {project.status === "archived" ? "Restore to Active" : "Archive Workspace"}
                                                                </DropdownMenuItem>
                                                                <DropdownMenuSeparator className="my-2 opacity-10" />
                                                                <DropdownMenuItem
                                                                    onClick={(e: React.MouseEvent) => { e.stopPropagation(); confirmDeleteProject(project.id); }}
