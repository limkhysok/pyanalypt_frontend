...
export function Footer() {
    return (
        <footer className="relative bg-background border-t border-border/80 pt-10 pb-10 overflow-hidden selection:bg-foreground/10">
            
            {/* Precision Blueprint Grid Backdrop */}
            <div className="absolute inset-0 bg-[linear-gradient(to_right,#80808008_1px,transparent_1px),linear-gradient(to_bottom,#80808008_1px,transparent_1px)] bg-[size:40px_40px] pointer-events-none opacity-30" />
            
            <div className="container relative z-10 mx-auto px-6 max-w-7xl">
                <div className="flex flex-col md:flex-row items-center justify-between gap-10 md:gap-4">

                    {/* ── [01] Brand Identity ── */}
                    <div className="flex items-center gap-6 group shrink-0">
                        <Link href="/" className="flex items-center gap-3 group/logo relative">
                            ...
                            <div className="flex flex-col -gap-0.5">
                                <span className="text-sm font-black tracking-tighter text-foreground uppercase opacity-80">
                                    PyAnalypt
                                </span>
                        ...
                        <p className="hidden md:block text-[9px] font-bold text-muted-foreground uppercase tracking-widest opacity-40 max-w-[140px] leading-tight">
                            Orchestrating boardroom-ready intelligence.
                        </p>
                    </div>

                    {/* ── [02] Navigation HUD ── */}
                    <nav aria-label="Footer" className="md:px-2">
                        <ul className="flex flex-wrap items-center justify-center gap-x-8 gap-y-4 text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">
                            ...
                            <li><Link href="/playground" className="hover:text-foreground transition-all flex items-center gap-2 group/link"><FlaskConical size={11} className="opacity-40 group-hover/link:opacity-100 transition-opacity" /> Play</Link></li>
                            <li><Link href="/about" className="hover:text-foreground transition-all flex items-center gap-2 group/link"><Info size={11} className="opacity-40 group-hover/link:opacity-100 transition-opacity" /> Intel</Link></li>
                        </ul>
                    </nav>
                    ...
                    <div className="flex items-center gap-8 md:gap-10 shrink-0">
                        ...
                        <div className="flex flex-col items-end gap-1.5 pt-0.5">
                                ...
                                <div className="flex items-center gap-2 opacity-20 cursor-default">
                                    ...
                                    <span className="text-[7px] font-black uppercase tracking-widest text-foreground">Operational Nom.</span>
                                </div>
                        </div>
                    </div>
            ...
            <div className="absolute bottom-0 inset-x-0 h-[3px] bg-linear-to-r from-transparent via-foreground/5 to-transparent pointer-events-none" />
        </footer>
    );
}
