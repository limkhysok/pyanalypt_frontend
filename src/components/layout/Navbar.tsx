    ...
    return (
        <div className="fixed top-0 inset-x-0 z-50 flex justify-center pt-6 px-6 pointer-events-none">
            <motion.div
                ...
                className={cn(
                    "relative pointer-events-auto flex items-center justify-between pl-4 pr-3 h-14 transition-all duration-700 rounded-full border-2 border-black dark:border-white/20 mx-auto w-full max-w-[1300px]",
                    scrolled
                ...
                <div className="flex items-center gap-2 shrink-0">
                    {!isLoading && isAuthenticated ? (
                        <DropdownMenu modal={false}>
                            <DropdownMenuTrigger asChild>
                                <Button variant="ghost" className="relative flex items-center gap-2.5 h-9 rounded-full px-2 hover:bg-blue-500/5">
                                    ...
                                    <span className="hidden sm:flex flex-col items-end leading-tight">
                                        ...
                                        <span className="text-[11px] font-black tracking-tight text-foreground/80 max-w-[100px] truncate">
                            ...
                            <DropdownMenuContent className="w-56 rounded-[2rem] p-2 bg-background/90 backdrop-blur-2xl border-border/10 shadow-2xl" align="end">
                    ...
                    <Button variant="ghost" size="icon" className="lg:hidden h-9 w-9 rounded-full" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
                ...
                >
                    {[
                        ...
                        <Link
                            ...
                            className="p-4 hover:bg-blue-500/5 rounded-[1.5rem] text-muted-foreground hover:text-blue-500 transition-all flex items-center justify-center text-xs font-black tracking-widest uppercase"
                        >
                            {item.label}
                        </Link>
                    ))}
                    ...
                    {isAuthenticated ? (
                        <div className="space-y-1">
                            ...
                            <button
                                ...
                                className="w-full p-4 hover:bg-red-500/5 rounded-[1.5rem] text-red-500/70 hover:text-red-500 transition-all flex items-center justify-center text-xs font-black tracking-widest uppercase"
                            >
                                Log out
                            </button>
                        </div>
                        ...
                        <div className="flex gap-2 px-2 pb-2">
                            <Link
                                href="/login"
                                onClick={() => setMobileMenuOpen(false)}
                                className="flex-1 p-4 hover:bg-blue-500/5 rounded-[1.5rem] text-muted-foreground hover:text-blue-500 transition-all flex items-center justify-center text-xs font-black tracking-widest uppercase border border-border/20"
                            >
                                LOGIN
                            </Link>
                            <Link
                                href="/register"
                                onClick={() => setMobileMenuOpen(false)}
                                className="flex-1 p-4 bg-blue-600 hover:bg-blue-700 rounded-[1.5rem] text-white transition-all flex items-center justify-center text-xs font-black tracking-widest uppercase"
                            >
                                REGISTER
                            </Link>
                        </div>
