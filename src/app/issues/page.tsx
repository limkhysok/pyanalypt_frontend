...
type SidePanelType = "overview" | "preview" | "issues";
...
export default function IssuesPage() {
    ...
    const [sidePanel, setSidePanel] = React.useState<SidePanelType>("overview");
                ...
                <table className="text-left border-collapse">
                    ...
                    <tbody>
                        {previewRows.map((row, rowIndex) => {
                            // Try to find a unique ID for the key, fallback to index
                            const rowId = (row.id as string | number) || (row.ID as string | number) || rowIndex;
                            return (
                                <tr key={`preview-row-${rowId}`} className="border-b border-border/10 hover:bg-primary/5">
                                    {previewColumns.map((column) => {
                                        const value = row[column];
                                        // Handle objects securely to avoid [object Object]
                                        ...
                                        if (value === null || value === undefined) {
                                            ...
                                            // Fallback for arrays, objects, etc.
...
interface IssuesContentProps {
    ...
    setSidePanel: (p: SidePanelType) => void;
...
function IssuesList({ issuesByColumn, searchQuery, renderIssueRows, filteredIssues }: Readonly<IssuesListProps>) {
    const columnNames = Object.keys(issuesByColumn);
